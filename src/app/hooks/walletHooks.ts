...
import type { ApprovalPayload } from '@/entrypoints/background/service/types';

export const useApproval = () => {
  const wallet = useWallet();
  const navigate = useNavigate();
  const getApproval: () => Promise<ApprovalPayload | null> = wallet.getApproval;
...
export const useRSA = () => {
  const [keys, setKeys] = useState<Keys>({ publicKey: '', privateKey: '' });
